...
export default function Today() {
  ...
  const { percentageDone, setPercentageDone } = useContext(
    PercentageDoneContext
  );
    ...
    <Container>
      ...
      <Day>
        ...
        {percentageDone === 0 || percentageDone === isNaN ? (
          <DayProgrss hasColor={false}>
            Nenhum hábito concluído ainda
          </DayProgrss>
        ) : (
          <DayProgrss hasColor={true}>{`${percentageDone.toFixed(
            0
          )}% do hábitos concluídos`}</DayProgrss>
        )}
      </Day>
      <Habits>
        {todayHabits.map(
          ({ id, name, done, currentSequence, highestSequence }) => (
            <Habit>
              <p>{name}</p>
              <div>
                <Sequence hasColor={done}>
                  Sequência atual: <strong>{`${currentSequence} dias`}</strong>
                </Sequence>
                <br />
                <Sequence
                  hasColor={done && currentSequence === highestSequence}
                >
