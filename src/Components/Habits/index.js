...
export default function Habits() {
  ...
  function handleSubmit(e) {
    ...
    const promisePostHabits = axios.post(
      "https://mock-api.bootcamp.respondeai.com.br/api/v2/trackit/habits",
      body,
      config
    );
  ...
  function deleteHabit(id) {
    if (window.confirm("Você deseja apagar o hábito?")) {
      ...
      const promiseDeleteHabits = axios.delete(
  ...
  return (
    <Container>
      ...
      <MyHabits>
        ...
        <Button
          image={plus}
          onClick={() => setHabitCriationEnabled(true)}
        ></Button>
      </MyHabits>
        ...
        <CreateHabit onSubmit={(e) => handleSubmit(e)}>
          <div>
            ...
            <div>
              {weekDays.map(({ indexDay, nameDay }) => (
                <Day
                  type="button"
                  onClick={() => addDay(indexDay)}
                  disabled={loading}
                  wasClicked={habitDays.includes(indexDay) ? true : false}
                >
                  {nameDay}
                </Day>
              ))}
          ...
          <FooterButtons>
            <SubmitButton
              disabled={loading}
              type="button"
              onClick={() => setHabitCriationEnabled(false)}
            >
              Cancelar
            </SubmitButton>
        ...
        <HabitsList>
          {habits.map(({ id, name, days }) => (
            <Habit>
              ...
              <div>
                {weekDays.map(({ indexDay, nameDay }) => (
                  <Day wasClicked={days.includes(indexDay) ? true : false}>
                    {nameDay}
                  </Day>
                ))}
              </div>
